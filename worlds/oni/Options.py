...
class Prehistoric(Toggle):
    """
    Enable Prehistoric DLC
    """
    display_name = "Enable Prehistoric DLC"

class Aquatic(Toggle):
    """
    Enable Aquatic DLC
    """
    display_name = "Enable Aquatic DLC"
...
class ClusterBase(Choice):
    ...
    def from_text(cls, text: str) -> Choice:
        text = text.lower()
        if text == "random":
            choice_list = [option for option in cls.name_lookup if option < 50]
