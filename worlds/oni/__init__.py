...
import Utils
from BaseClasses import Item, Location, Tutorial, Region, ItemClassification, Entrance
...
def convert_ap_class(ap_string):
    ...
    match ap_string:
        ...
        case "Deprioritized":
            ap_class = ItemClassification.deprioritized
        case "ProgressionDeprioritizedSkipBalancing":
            ap_class = ItemClassification.progression_deprioritized_skip_balancing
        case "ProgressionDeprioritized":
            ap_class = ItemClassification.progression_deprioritized
    return ap_class
...
class ONIWorld(World):
    ...
    advanced_loc_names = ["Agriculture", "Ranching", "Fossil Fuels", "Sound Amplifiers", "Advanced Power Regulation", "Smart Storage", "Refined Renovations",
                          "Artificial Friends", "Notification Systems", "Pathogen Diagnostics", "Liquid-Based Refinement Processes", "Flow Redirection"
                         "Distillation", "Improved Plumbing", "Temperature Modulation", "Decontamination", "Improved Ventilation", "Textile Production", 
                         "Fine Art", "Generic Sensors"]
    ...
    def generate_early(self) -> None:
        ...
        if "ceres" in planet and not self.frosty:
            logging.warning(f"Frosty DLC is not enabled and planet choice requires it. Changing option to default planet.")
            if self.spaced_out:
                self.options.cluster.value = 0         # set to default for spaced out
            else:
                self.options.cluster_base.value = 0         # set to default for base game

        if "relica" in planet and not self.prehistoric:
            logging.warning(f"Prehistoric DLC is not enabled and planet choice requires it. Changing option to default planet.")
            if self.spaced_out:
                self.options.cluster.value = 0         # set to default for spaced out
            else:
                self.options.cluster_base.value = 0         # set to default for base game

        if "marinea" in planet and not self.aquatic:
            logging.warning(f"Aquatic DLC is not enabled and planet choice requires it. Changing option to default planet.")
            if self.spaced_out:
                self.options.cluster.value = 0         # set to default for spaced out
            else:
                self.options.cluster_base.value = 0         # set to default for base game
        ...
        for item in self.default_item_list:
            ...
            if self.prehistoric == False and item.version == "Prehistoric":
                continue;
            if self.aquatic == False and (item.version == "Aquatic" or item.version == "BioAqua"):
                continue;
            ...
            match research_level:
                ...
                case "advanced":
                    count = 1
                    if tech in self.advanced_loc_names:
                        ...
                        for location in advanced_late_locations:
                            tech_name = location.split(" - ")[0]
                            if tech == tech_name:
                                count += 1
                        location_name = f"{tech} - {count}"
                        advanced_late_locations.append(location_name)
                case "radbolt":
                    count = 1
        ...
        if self.base_only == True:
            self.all_regions = [
                ...
                RegionInfo(RegionNames.AdvancedLate, advanced_late_locations),
                RegionInfo(RegionNames.Space_Base, orbital_locations)
            ]
            self.all_locations = basic_locations + advanced_locations + advanced_late_locations + orbital_locations
        else:
            self.all_regions = [
                RegionInfo("Menu", []),
                RegionInfo(RegionNames.Basic, basic_locations),
            ...
            self.all_locations = basic_locations + advanced_locations + advanced_late_locations + radbolt_locations + orbital_locations
    ...
    def create_regions(self) -> None:
        ...
        for region_info in self.all_regions:
            ...
            self.regions_by_name[region_info.name] = region
    ...
    def create_items(self) -> None:
        ...
        '''json_string = json.dumps(self.all_locations, default=lambda o: o.__dict__, indent=4)
        output_file_path = os.path.join(Utils.user_path("data", "ONI"), f"..\\locations.json")
        with open(output_file_path, "w") as file:
            file.write(json_string)'''
    ...
    def set_rules(self) -> None:
        ...
        if self.base_only == True:
            self.logic_options["planet"] = f"{self.options.cluster_base.current_key}_base"
            self.regions_by_name["Menu"].connect(
                self.regions_by_name[RegionNames.Basic], None, None)
            self.regions_by_name[RegionNames.Basic].connect(
                self.regions_by_name[RegionNames.Advanced], None, lambda state: can_advanced_research(self.player, self.internal_item_to_name, state, self.logic_options))
            self.regions_by_name[RegionNames.Advanced].connect(
                self.regions_by_name[RegionNames.AdvancedLate], None, lambda state: can_advanced_late_research(self.player, self.internal_item_to_name, state, self.logic_options))
            self.regions_by_name[RegionNames.AdvancedLate].connect(
                self.regions_by_name[RegionNames.Space_Base], None, lambda state: can_space_research_base(self.player, self.internal_item_to_name, state, self.logic_options))
            
            print(self.regions_by_name)
            
            self.set_completion_rule(lambda state: state.can_reach_region(RegionNames.Space_Base, self.player))
        else:
            self.regions_by_name["Menu"].connect(
                self.regions_by_name[RegionNames.Basic], None, None)
            self.regions_by_name[RegionNames.Basic].connect(
            ...
            self.regions_by_name[RegionNames.Advanced].connect(
                self.regions_by_name[RegionNames.AdvancedLate], None, lambda state: can_advanced_late_research(self.player, self.internal_item_to_name, state, self.logic_options))
            self.regions_by_name[RegionNames.AdvancedLate].connect(
                self.regions_by_name[RegionNames.Nuclear], None, lambda state: can_nuclear_research(self.player, self.internal_item_to_name, state, self.logic_options))
            self.regions_by_name[RegionNames.Nuclear].connect(
                self.regions_by_name[RegionNames.Space_DLC], None, lambda state: can_space_research(self.player, self.internal_item_to_name, state, self.logic_options))
            self.set_completion_rule(lambda state: state.can_reach_region(RegionNames.Space_DLC, self.player))

        #if self.multiworld.players == 1:
        if self.base_only == True:
            # 1. Gather all locations in your early regions (Basic and Advanced)
            '''early_locations = []
            for region_name in [RegionNames.Basic, RegionNames.Advanced, RegionNames.AdvancedLate]:
                region = self.multiworld.get_region(region_name, self.player)
                # Only grab locations that don't already have an item placed in them
                early_locations.extend([loc for loc in region.locations if loc.item is None])'''
    
            # Core research gates that always need to be placed early
            required_items = [
                self.internal_item_to_name["CosmicResearchCenter"],
                self.internal_item_to_name["ResearchModule"],
                self.internal_item_to_name["Telescope"],
                self.internal_item_to_name["CommandModule"]
            ]

            # 3. Seeded Random Choice: Decide which rocket path this specific seed will use
            # self.random guarantees that the choice is identical if the seed is identical
            rocket_path = self.random.choice(["steam", "advanced"])

            if rocket_path == "steam":
                # Path A requires the Steam Engine
                print(f"Seed {self.multiworld.seed} is using the Steam Engine path")
                required_items.append(self.internal_item_to_name["SteamEngine"])
            else:
                print(f"Seed {self.multiworld.seed} is using the Advanced Engine path")
                # Path B requires an Advanced Engine, Fuel, and exactly ONE random Oxidizer
                adv_engine = self.random.choice([
                    self.internal_item_to_name["KeroseneEngine"], 
                    self.internal_item_to_name["HydrogenEngine"], 
                    self.internal_item_to_name["BiodieselEngine"]
                ])
                oxidizer = self.random.choice([
                    self.internal_item_to_name["OxidizerTank"], 
                    self.internal_item_to_name["OxidizerTankLiquid"]
                ])
                required_items.extend([adv_engine, self.internal_item_to_name["LiquidFuelTank"], oxidizer])


            for pool_item in self.multiworld.itempool:
                    if pool_item.name in required_items and pool_item.player == self.player:
                        pool_item.classification = ItemClassification.progression
                        break
            '''
            # 4. Randomly shuffle our early locations using the seed
            self.random.shuffle(early_locations)

            # 5. Lock the chosen progression items into the randomly selected early locations
            for item_name in required_items:
                # Pull the next available random location from the shuffled list
                target_location = early_locations.pop(0)
        
                # Create the actual item object from your world
                item_object = self.create_item(item_name)
        
                # Lock it in place. The generator will now bypass this slot entirely
                target_location.place_locked_item(item_object)
                self.multiworld.state.collect(item_object, prevent_sweep=False, location=target_location)

                print(f"Locked {item_name} into {target_location.name} for player {self.player}")
        
                # Remove this item from the global item pool so it isn't generated a second time
                for pool_item in self.multiworld.itempool:
                    if pool_item.name == item_name and pool_item.player == self.player:
                        self.multiworld.itempool.remove(pool_item)
                        break'''

            #self.set_completion_rule(lambda state: can_advanced_late_research(self.player, self.internal_item_to_name, state, self.logic_options))
