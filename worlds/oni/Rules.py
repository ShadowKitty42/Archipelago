...
def can_advanced_late_research(player, item_list: Dict, state: CollectionState, player_options: Dict) -> bool:
    # Need to be able to actually do the research, and handle liquids and gas
    return can_advanced_research(player, item_list, state, player_options) and can_manage_liquid(player, item_list, state) and can_manage_gas(player, item_list, state)
...
def can_survive_basic(player, item_list: Dict, state: CollectionState, player_options: Dict) -> bool:
    ...
    if on_aqua_planet(player_options):
        running_state = running_state and state.has_all({item_list["PlasticGasket"], item_list["WideFarmTile"], item_list["ReefGenerator"]}, player)
    return running_state

def can_reach_space_base(player, item_list: Dict, state: CollectionState) -> bool:
    # Command Capsule is non-negotiable
    running_state = state.has(item_list["CommandModule"], player)
    # Has any engine and fuel tank
    running_state = running_state and (state.has(item_list["SteamEngine"], player) or \
        (state.has_any({item_list["KeroseneEngine"], item_list["HydrogenEngine"], item_list["BiodieselEngine"]}, player) and \
...
def can_reach_space(player, item_list: Dict, state: CollectionState) -> bool:
    ...
    running_state = running_state and (state.has_any({item_list["CO2Engine"], item_list["HEPEngine"], 
       ...
       (state.has_any({item_list["KeroseneEngineCluster"], item_list["HydrogenEngineCluster"], item_list["BiodieselEngineCluster"]}, player) and \
...
def can_manage_liquid(player, item_list: Dict, state: CollectionState) -> bool:
    ...
    # Water Sieve
    running_state = running_state and state.has(item_list["WaterPurifier"], player)
    return running_state
...
def on_aqua_planet(options: Dict) -> bool:
    if options["planet"].startswith("marinea"):
        return True
    return False